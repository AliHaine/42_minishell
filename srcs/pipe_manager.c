...
static bool	execv_helper(t_t_i ti, t_cmds *cmds, int pipes[][2])
{
	if (!is_unused(cmds))
		return (false);
	if (cmds->w == 0)
...
static bool	pipe_brain2(t_t_i ti, int *pid, t_cmds *cmds, int pipes[][2])
{
	if (ti.a == (ti.c - 1))
	{
		if (pid[ti.a] == 0)
			if (!execv_helper(ti, cmds, pipes))
				return (false);
...
static bool	pipe_brain(t_t_i ti, pid_t *pid, t_cmds *cmds)
{
	int				pipes[19][2];
	int				r;
	...
	while (cmds)
	{
		...
		if (ti.a == 0)
		{
			...
			if (!pipe_brain2(ti, pid, cmds, pipes))
				return (false);
		cmds = cmds->next;
